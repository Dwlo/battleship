...
(defroutes app-routes
  ...
  (GET "/games/:game-id/battlefield" [game-id] (response (log/show-battlefield (deref (gm/lookup-game game-id)))))

  ;; --- Retrieves the full battlefield data for the given game id.
  (GET "/games/:game-id/show-enemies" [game-id] (response (log/show-enemiesX (deref (gm/lookup-game game-id)))))
  ...
  (PUT "/games/:game-id/players/:player/fire" {{row :row col :col player :player game-id :game-id} :params}
       (if-let [game (gm/lookup-game game-id)]
         (let [r (read-string row)
               c (read-string col)
               g (gm/lookup-game game-id)]
           (response {:shot-result (log/attack r c player g)}))
  ...
  (GET "/games/:game-id/stats" [game-id]
       (if-let [game (deref (gm/lookup-game game-id))]
